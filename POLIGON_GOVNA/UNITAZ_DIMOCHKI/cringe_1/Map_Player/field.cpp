    ...
    if (!def) {
        ...
        if (inp == 'y') {
            ...
            if (logger->logging(Info))
                logger->printLog(Info, "Player created\n");
        }
        else {
...
void Field::movePlayer(int x, int y) {
    ...
    if (player_x == 0 && x == -1) {
        if (map[player_y][size_x - 1].isPassable()) {
            player_x = size_x - 1;
            if (map[player_y][player_x].haveEvent()) {
        ...
        else if (logger->logging(Error))
            logger->printLog(Error, "Cell isn't passable\n");
    }
    else if (player_y == 0 && y == -1) {
        ...
        else if (logger->logging(Error) == true)
            logger->printLog(Error, "Cell isn't passable\n");
    }
    else {
